...
func TestUserUsecase_RestoreUser(t *testing.T) {
	...
	t.Run("should return error service_api error when restore user", func(t *testing.T) {
		// Mock find deleted user success
		repo.User.EXPECT().FindDeletedByID(ctx, ids.UserID).Return(mocks.User, nil).Times(1)
	...
	t.Run("should return error service_api error when find restored user", func(t *testing.T) {
		// Mock find deleted user success
		repo.User.EXPECT().FindDeletedByID(ctx, ids.UserID).Return(mocks.User, nil).Times(1)
		// Mock restore user success
		repo.User.EXPECT().Restore(ctx, ids.UserID).Return(nil).Times(1)
	...
	t.Run("should return error service_api error when cache delete fails", func(t *testing.T) {
		// Mock find deleted user success
		repo.User.EXPECT().FindDeletedByID(ctx, ids.UserID).Return(mocks.User, nil).Times(1)
		// Mock restore user success
		repo.User.EXPECT().Restore(ctx, ids.UserID).Return(nil).Times(1)
