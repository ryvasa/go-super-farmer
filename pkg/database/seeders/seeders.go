...
func SeedLandCommodities(db *gorm.DB, lands []domain.Land, commodities []domain.Commodity, count int) {
	gofakeit.Seed(0)
	for i := 0; i < count; i++ {
...
func Seeders(db *gorm.DB) {
	...
	SeedLands := SeedLands(db, users, 10)
	SeedLandCommodities(db, SeedLands, commodities, 10)
	SeedProvinces := SeedProvinces(db, 10)
	SeedCities := SeedCities(db, SeedProvinces, 5)
	SeedDemand := SeedDemand(db, commodities, SeedCities, 10)
	SeedDemandHistory(db, SeedDemand, 10)
	SeedPrices := SeedPrices(db, commodities, SeedCities, 10)
	SeedPriceHistory(db, SeedPrices, 10)
	SeedSupply := SeedSupply(db, commodities, SeedCities, 10)
	SeedSupplyHistory(db, SeedSupply, 10)
