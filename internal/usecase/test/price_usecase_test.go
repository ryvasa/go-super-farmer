...
func TestPriceUsecase_DownloadPriceHistoryByCommodityIDAndCityID(t *testing.T) {
	...
	t.Run("should return error when get price by commodity id and city id", func(t *testing.T) {
		repo.Price.EXPECT().FindByCommodityIDAndCityID(context.Background(), dtos.Params.CommodityID, dtos.Params.CityID).Return(nil, utils.NewInternalError("internal error")).Times(1)
