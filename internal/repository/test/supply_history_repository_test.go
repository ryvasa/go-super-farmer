...
func SupplyHistoryRepositorySetup(t *testing.T) (*sql.DB, sqlmock.Sqlmock, repository.SupplyHistoryRepository, SupplyHistoryIDs, SupplyHistoryMockRows, SupplyHistoryMocDomain) {

	sqlDB, db, mock := database.DbMock(t)

	repo := repository.NewSupplyHistoryRepository(db)
...
func TestSupplyHistoryRepository_Create(t *testing.T) {
	...
	expectedSQL := `INSERT INTO "supply_histories" ("id","commodity_id","region_id","quantity","created_at","updated_at","deleted_at") VALUES ($1,$2,$3,$4,$5,$6,$7)`

	t.Run("should not return error when create successfully", func(t *testing.T) {
		mock.ExpectBegin()
		mock.ExpectExec(regexp.QuoteMeta(expectedSQL)).
			WithArgs(ids.SupplyHistoryID, ids.CommodityID, ids.RegionID, float64(10), sqlmock.AnyArg(), sqlmock.AnyArg(), nil).
