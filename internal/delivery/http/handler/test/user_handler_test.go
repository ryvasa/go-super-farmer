...
func TestUserHandler_RegisterUser(t *testing.T) {
	...
	t.Run("should return error when internal error", func(t *testing.T) {
		uc.User.EXPECT().Register(gomock.Any(), gomock.Any()).Return(nil, utils.NewInternalError("internal error")).Times(1)
		reqBody := `{"email":"test@example.com"}`
		req, _ := http.NewRequest(http.MethodPost, "/users", bytes.NewReader([]byte(reqBody)))
		req.Header.Set("Content-Type", "application/json")
		w := httptest.NewRecorder()
...
func TestUserHandler_GetOneUser(t *testing.T) {
	...
	t.Run("should return error when usecase error", func(t *testing.T) {
		...
		uc.User.EXPECT().GetUserByID(gomock.Any(), ids.UserID).Return(nil, utils.NewInternalError("internal error")).Times(1)
		req, _ := http.NewRequest(http.MethodGet, "/users/"+ids.UserID.String(), nil)
		w := httptest.NewRecorder()
		r.ServeHTTP(w, req)
...
func TestUserHandler_UpdateUser(t *testing.T) {
	...
	t.Run("should return error when usecase error", func(t *testing.T) {
		uc.AutUtil.EXPECT().GetAuthRole(gomock.Any()).Return("Admin", nil).Times(1)
		uc.User.EXPECT().UpdateUser(gomock.Any(), ids.UserID, "Admin", gomock.Any()).Return(nil, utils.NewInternalError("internal error")).Times(1)
...
func TestUserHandler_DeleteUser(t *testing.T) {
	...
	t.Run("should return error when usecase error", func(t *testing.T) {
		uc.User.EXPECT().DeleteUser(gomock.Any(), ids.UserID).Return(utils.NewInternalError("internal error")).Times(1)
		req, _ := http.NewRequest(http.MethodDelete, "/users/"+ids.UserID.String(), nil)
		w := httptest.NewRecorder()
		r.ServeHTTP(w, req)
...
func TestUserHandler_RestoreUser(t *testing.T) {
	...
	t.Run("should return error when usecase error", func(t *testing.T) {
		...
		uc.User.EXPECT().RestoreUser(gomock.Any(), ids.UserID).Return(nil, utils.NewInternalError("internal error")).Times(1)
		req, _ := http.NewRequest(http.MethodPost, "/users/"+ids.UserID.String()+"/restore", nil)
		w := httptest.NewRecorder()
		r.ServeHTTP(w, req)
